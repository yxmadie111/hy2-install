...
install_deps() {
    ...
    if command -v apt &>/dev/null; then
        ...
        apt install -y curl wget openssl iptables socat cron &>/dev/null
        systemctl enable cron &>/dev/null 2>&1
        systemctl start cron &>/dev/null 2>&1
    elif command -v yum &>/dev/null; then
        yum install -y curl wget openssl iptables socat cronie &>/dev/null
...
show_user_links() {
    ...
    local server_ip port masquerade domain insecure sni_value
    ...
    while IFS=: read -r uname upw; do
        if [[ "$user_count" -le 1 ]]; then
            # 单用户 password 模式
            local url="hysteria2://${upw}@${server_ip}:${port}/?insecure=${insecure}&sni=${sni_value}#Hy2-${uname}"
        else
            # 多用户 userpass 模式
            local url="hysteria2://${uname}:${upw}@${server_ip}:${port}/?insecure=${insecure}&sni=${sni_value}#Hy2-${uname}"
        fi
        ...
        if [[ -f "$HY2_HOPPING_CONF" ]]; then
            source "$HY2_HOPPING_CONF"
            if [[ "$user_count" -le 1 ]]; then
                local hop_url="hysteria2://${upw}@${server_ip}:${port}/?insecure=${insecure}&sni=${sni_value}&mport=${HOP_START}-${HOP_END}#Hy2-Hop-${uname}"
            else
                local hop_url="hysteria2://${uname}:${upw}@${server_ip}:${port}/?insecure=${insecure}&sni=${sni_value}&mport=${HOP_START}-${HOP_END}#Hy2-Hop-${uname}"
            fi
...
# 根据 users.txt 和 meta.conf 重新生成 config.yaml
rebuild_config() {
    if [[ ! -f "$HY2_META" ]]; then
...
show_client_info() {
    ...
    while IFS=: read -r uname upw; do
        if [[ "$user_count" -le 1 ]]; then
            local url="hysteria2://${upw}@${ip}:${port}/?insecure=${insecure}&sni=${sni_value}#Hy2-${uname}"
        else
            local url="hysteria2://${uname}:${upw}@${ip}:${port}/?insecure=${insecure}&sni=${sni_value}#Hy2-${uname}"
        fi
...
do_install() {
    ...
    if [[ "$domain_choice" == "1" ]]; then
        ...
        if [[ -z "$domain" ]]; then
            ...
            msg_info "检查域名解析..."
            local resolved_ip
            resolved_ip=$(curl -s --max-time 5 "https://dns.google/resolve?name=${domain}&type=A" 2>/dev/null | grep -oP '"data"\s*:\s*"\K[^"]+' | head -1)

            if [[ "$resolved_ip" == "$server_ip" ]]; then
                msg_ok "域名 ${domain} 已正确指向 ${server_ip}"
            else
                msg_warn "域名解析结果 (${resolved_ip:-无}) 与本机 IP (${server_ip}) 不一致"
...
EOF
    ...
    show_client_info "$server_ip" "$port" "$masquerade" "$domain"
}
...
show_menu() {
    ...
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════╗${RESET}"
    echo -e "${GREEN}║      Hysteria 2 一键管理脚本 v2.0       ║${RESET}"
    echo -e "${GREEN}╚══════════════════════════════════════════╝${RESET}"
    echo ""
